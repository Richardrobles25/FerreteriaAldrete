...
if (isset($_GET['detalle_venta'])) {
    // [FIX] sucursal=0 ("Todas las sucursales") no debe impedir ver el detalle de una venta
...
// [FIX] sucursal=0 ("Todas las sucursales") no debe eliminar todas las ventas — antes exigia
// "ca.sucursal_id = 0", que ninguna caja real cumple.
$where  = "WHERE 1=1";
$params = [];
if ($sucursalVista !== 0) { $where .= " AND ca.sucursal_id = ?"; $params[] = $sucursalVista; }
...
$whereM  = "WHERE 1=1";
$paramsM = [];
if ($sucursalVista !== 0) { $whereM .= " AND m.sucursal_id = ?"; $paramsM[] = $sucursalVista; }
if ($fechaDesde !== '' && $fechaHasta !== '') {
...
// [FIX] sucursal=0 ("Todas las sucursales") no debe eliminar todas las devoluciones — antes
// exigia "ca.sucursal_id = 0", que ninguna caja real cumple.
$condSucDev     = ($sucursalVista !== 0) ? ' AND ca.sucursal_id = ?' : '';
$paramsSucDev   = ($sucursalVista !== 0) ? [$sucursalVista] : [];
$whereDevFecha  = '';
$paramsDevFecha = $paramsSucDev;
...
if ($fechaDesde !== '' && $fechaHasta !== '') {
    ...
    $paramsDevFecha = array_merge($paramsSucDev, [$fechaDesde, $fechaHasta, $fechaDesde, $fechaHasta]);
} elseif ($fechaDesde !== '') {
    $whereDevFecha  = 'AND (DATE(d.procesada_en) >= ? OR (d.cancelada_en IS NOT NULL AND DATE(d.cancelada_en) >= ?))';
    $paramsDevFecha = array_merge($paramsSucDev, [$fechaDesde, $fechaDesde]);
} elseif ($fechaHasta !== '') {
    $whereDevFecha  = 'AND (DATE(d.procesada_en) <= ? OR (d.cancelada_en IS NOT NULL AND DATE(d.cancelada_en) <= ?))';
    $paramsDevFecha = array_merge($paramsSucDev, [$fechaHasta, $fechaHasta]);
}
...
<div class="main">
    <div class="topbar">
        <div class="topbar-left">
            ...
            <h2>Historial de Ventas — <?= htmlspecialchars($nombreSucursalVista) ?></h2>
        </div>
        ...
        <form method="GET">
            <div class="filtros">
                ...
                <?php if ($metodo || $estado || $buscar || $fechaDesde || $fechaHasta): ?>
                    <a class="btn-limpiar" href="cajero_historialVentas.php">Limpiar</a>
                <?php endif; ?>
            </div>
        </form>
