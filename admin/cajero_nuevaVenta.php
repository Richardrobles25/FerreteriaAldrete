...
// ── AJAX: todos los productos (precarga) ─────────────────────────────────────
if (isset($_GET['get_productos_all'])) {
    $stmt = $pdo->prepare("
        SELECT p.producto_id, p.codigo, p.nombre_producto, p.precio_venta,
               p.precio_mayoreo, ss.stock_actual, p.tipo_venta, ss.sucursal_id
        FROM productos p
        INNER JOIN stock_sucursal ss ON ss.producto_id = p.producto_id AND ss.activo = 1
        WHERE p.activo = 1
...
// ── AJAX: todos los clientes (precarga) ──────────────────────────────────────
...
if (isset($_GET['get_paquetes'])) {
    ...
    foreach ($filas as $f) {
        ...
        $cantReq   = floatval($f['cantidad_requerida']);
        $stockActual = floatval($f['stock_actual']);
        ...
        if ($stockActual < $cantReq) {
            $agrupados[$pid]['disponible'] = false;
        }
    }
    header('Content-Type: application/json');
...
if (isset($_GET['inventario_sucursal'])) {
    ...
    $stmt = $pdo->prepare("SELECT p.producto_id, p.codigo, p.nombre_producto, ss.stock_actual, p.precio_venta, p.tipo_venta FROM productos p INNER JOIN stock_sucursal ss ON ss.producto_id = p.producto_id $where ORDER BY p.nombre_producto ASC LIMIT 50");
    $stmt->execute($params);
    header('Content-Type: application/json');
    echo json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
    exit();
    ...
    if (!empty($items) && $metodo_pago) {
        ...
        if ($metodo_pago === 'Credito' && $cliente_id) {
            $pdo->prepare("INSERT INTO creditos (cliente_id,venta_id,monto_total,saldo_pendiente,estado,fecha_limite) VALUES (?,?,?,?,'Activo',?)")
                ->execute([$cliente_id,$venta_id,$total,$total,date('Y-m-d H:i:s', strtotime('+2 minutes'))]);
        }
...
<div class="main">
    ...
    <div class="content">
        <?php if (isset($_GET['msg']) && $_GET['msg'] === 'exito'): ?>
            <div class="msg-exito">
                <span>Venta registrada correctamente. <a href="cajero_nuevaVenta.php" style="color:#2e7d32;font-weight:600;">Nueva venta</a></span>
