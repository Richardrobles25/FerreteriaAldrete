...
// [FIX-CAJA-REDIRECT-AJAX] El redirect por falta de caja abierta ya NO vive aqui arriba —
// se aplica mas abajo, en cada punto que de verdad depende de $caja (el handler de
// movimiento_caja, con su propio guard porque responde por redirect igual que este, y
// confirmar_venta, que responde JSON cuando la peticion es AJAX). Un redirect aqui arriba se
// ejecutaba ANTES de que CUALQUIER endpoint AJAX de este archivo (busqueda de productos,
// clientes, paquetes, scanner, ticket, y sobre todo confirmar_venta) pudiera correr,
// rompiendo su contrato JSON — el fetch().then(r=>r.json()) del cliente intentaba parsear el
// HTML de abrirCaja.php como JSON y caia en el catch generico ("no se pudo procesar"), sin
// decirle al cajero la razon real. Reproducido en vivo: un usuario cuya caja se cierra en
// otra pestaña mientras esta sigue con nuevaVenta.php abierto recibia un 302 en vez de JSON
// al intentar cobrar. El redirect de pagina completa (carga normal sin AJAX) se aplica al
// final del archivo, justo antes del HTML — ver [FIX-CAJA-REDIRECT-AJAX] mas abajo.
// Mismo patron ya corregido en cajero_creditos.php.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['movimiento_caja'])) {
    // [FIX-CAJA-REDIRECT-AJAX] Este handler responde por redirect (no JSON), asi que un
    // redirect aqui por falta de caja es consistente con su propio contrato — a diferencia
    // de los endpoints AJAX de abajo, aqui SI corresponde cortar temprano.
    if (!$caja) {
        header('Location: cajero_abrirCaja.php?msg=sinCaja');
        exit();
    }
    // [AUTOFIX] BUG-04: Verificar CSRF token en handler de movimiento de caja
    requerirCSRF($_POST['_token'] ?? '', 'cajero_nuevaVenta.php');
    ...
    if ($montoMov > 500000) {
        ...
        $pdo->prepare("INSERT INTO movimientos_caja (caja_id, usuario_id, sucursal_id, tipo, monto, nota) VALUES (?,?,?,?,?,?)")
            ->execute([$caja['caja_id'], $_SESSION['usuario_id'], $sucursalVista, $tipoMov, $montoMov, $notaMov]);
        header('Location: cajero_nuevaVenta.php?msg_mov=' . $tipoMov);
        exit();
    } else {
        $msgMovCaja = 'error';
    }
...
");
$stmtPromos->execute([$sucursalVista, $sucursalVista]);
...
if (isset($_GET['get_paquetes'])) {
    // [FIX-ALTO-B-06] Antes no se filtraba pr.activo/ss.activo: un paquete cuyo
    // componente fue desactivado o retirado de esta sucursal seguia ofreciendose
    // (paquete fantasma). Filtrar solo la fila del JOIN no bastaba: el paquete seguia
    // apareciendo con menos productos de los que en realidad requiere. Se excluye el
    // paquete COMPLETO con un NOT EXISTS si cualquiera de sus componentes esta inactivo
    // o sin stock configurado en esta sucursal.
    $stmtPaq = $pdo->prepare("
    ...
    ");
    $stmtPaq->execute([$sucursalVista, $sucursalVista]);
...
if (isset($_GET['buscar_paquete'])) {
    ...
    // [FIX-ALTO-B-06] Ver nota en get_paquetes: se excluye el paquete completo (no solo
    // la fila) si algun componente esta inactivo o sin stock configurado en la sucursal.
    $stmt = $pdo->prepare("
        SELECT pk.paquete_id, pk.codigo, pk.nombre, pk.precio_paquete,
    ...
    ");
    $stmt->execute([$sucursalVista, '%'.$termino.'%', '%'.$termino.'%', $sucursalVista]);
...
if (isset($_GET['buscar_combo'])) {
    ...
    $sucursal_id = intval($sucursalVista);
    $like        = '%' . $termino . '%';
...
if (isset($_GET['inventario_sucursal'])) {
    ...
    // [FIX-MEDIO-D1-09] El selector de sucursal de este modal es intencional (consultar
    // stock de OTRA sucursal es una funcion real, ej. "mandar al cliente a la otra
    // tienda"), pero el JSON incluia precio_compra (costo) de esa otra sucursal aunque el
    // frontend solo lo usa cuando se agrega el producto al carrito — y eso solo es posible
    // para la sucursal PROPIA (el boton "Agregar" esta deshabilitado para "otra sucursal").
    // Se oculta precio_compra cuando la sucursal consultada no es la propia del usuario
    // (para Administrador, que ya ve todo, no aplica esta restriccion).
...
if (isset($_GET['ticket_venta'])) {
    ...
    $stmtV = $pdo->prepare("
        ...
        WHERE v.venta_id = ?
    ");
    $stmtV->execute([$sucursalVista, $venta_id]);
    $venta = $stmtV->fetch(PDO::FETCH_ASSOC);
    // [FIX-IDOR-TICKET] (espejo de cajeroInventario/nuevaVenta.php) La consulta de
    // venta_productos de abajo corria siempre, sin importar si $venta salio false por el
    // filtro de sucursal de arriba — un admin viendo otra sucursal podia pedir el ticket de
    // una venta ajena y recibir productos/cantidades/precios aunque folio/total/cliente si
    // quedaran bloqueados.
    if (!$venta) {
        header('Content-Type: application/json');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirmar_venta'])) {
    // [FIX-CAJA-REDIRECT-AJAX] Sin caja abierta (se cerro en otra pestaña/dispositivo
    // mientras esta seguia abierta) ya no se corta con un redirect antes de este bloque —
    // eso rompia el fetch().then(r=>r.json()) del formulario, que SIEMPRE manda _ajax=1.
    // Responder JSON aqui deja que el cajero vea el motivo real en vez de un error generico.
    if (!$caja) {
    ...
    // [FIX-REFERENCIA-TRANSFERENCIA] Igual que cajero_ventasPendientes.php: la pantalla ya
    // exige esta referencia antes de permitir "Cobrar" (ver validacion JS mas abajo), pero
    // el servidor nunca la revalidaba — probado en vivo: un POST directo con metodo_pago=
    ...
    if (!$errorVenta && !empty($items) && $metodo_pago) {
        ...
        try {
            ...
            foreach ($gruposPaq as $pqIdVal => $grupo) {
                ...
                // [FIX-ALTO-B-13] La comprobación de arriba solo valida la SUMA del grupo;
                // el reparto por producto seguía viniendo del cliente, así que un item podía
                // quedar en $0 (o negativo) mientras otro compensaba de más mantenendo el
                // total correcto — un producto "gratis" quedaba registrado en
                // venta_productos, explotable luego en devoluciones individuales o reportes
                // de costo/comisión. Se recalcula el precio de cada item en el servidor con
                // el mismo reparto proporcional que usa el POS (último item absorbe el
                // redondeo), ignorando el precio que mandó el cliente para estas filas.
                $totalCantPaq = 0;
            ...
            if ($cliente_id) {
                ...
                // cajeroInventario/nuevaVenta.php) el camino de Credito ya exige
            ...
            if ($metodo_pago === 'Credito' && $cliente_id) {
                ...
                // [FIX-ALTO-E-07] Antes solo se contaba deuda 'Activo': un credito que ya
                // paso su fecha_limite y se marco 'Vencido' dejaba de contar contra el
                // limite, y el cliente recuperaba cupo completo estando moroso.
            ...
            // [FIX-ALTO-D1-03] Antes se guardaban $subtotal/$descuento tal cual llegaron del
            // navegador: solo el TOTAL se validaba contra lo real ($totalEsperado), pero
            // subtotal/descuento podian quedar con cualquier valor que siguiera cuadrando la
            // ecuacion (un grado de libertad libre entre esos dos campos). Ese subtotal y
            // descuento corruptos alimentan directamente el calculo de reembolso proporcional
            // de Devoluciones (ventaInfo['venta_subtotal']/['venta_descuento']). Ahora se
            // guardan los valores ya recalculados y validados en el servidor: $sumaItems (la
            // suma real de los items) y $descuentoCliente (el descuento ya acotado al % del
            // cliente).
            $stmt = $pdo->prepare("
                INSERT INTO ventas
            ...
            foreach ($items as $idxVP => $item) {
                ...
                // [FIX-MEDIO-D1-07] Antes no se revalidaba que el producto siguiera activo al
                // momento de confirmar la venta: si el carrito se armo (y quedo persistido en
                // localStorage del navegador) ANTES de que alguien desactivara el producto,
                // nada en el servidor lo detectaba — el endpoint de busqueda si filtra
                // activo=1, pero confirmar_venta nunca revisaba el carrito ya armado contra el
                ...
                $stmtActivoChk = $pdo->prepare("
                    ...
                    WHERE p.producto_id = ?
                ");
                $stmtActivoChk->execute([$sucursalVista, $item['producto_id']]);
                ...
                // [FIX-MEDIO-D1-11] Antes el piso solo era estricto (100% del costo) cuando el
                // item traia nota_ajuste; sin nota, el piso bajaba a 50% — y como el cliente
                // decide si manda la nota o no, cualquiera podia esquivar el piso estricto
                // con solo dejar la nota vacia, sin importar si el descuento era real. Ahora
                // el piso SIEMPRE es el 100% del costo. La unica excepcion valida es un precio
                // que coincide con una promocion activa real de esta sucursal (ya se valido
                // por separado contra precio_venta y el minimo de $0.01 al crearse — ver
                // promociones.php) — una promocion SI puede ir por debajo del costo a
                // proposito, pero solo si es exactamente el precio que el admin autorizo.
                if (empty($paqId)) {
                    ...
                    ");
                    $stmtPromoChk->execute([$item['producto_id'], $sucursalVista]);
                    ...
                    // [FIX-CRIT-D1-02] Antes, cuando precio_compra = $0 (17 de 46 productos reales
                    // del catálogo al momento de la auditoría), el "if ($precioCompraDB > 0)" saltaba
                    // TODA la validación y el precio del cliente se aceptaba tal cual — incluyendo
                    // $0.00. Ahora, si no hay precio_compra capturado, se usa como referencia el
                    // precio de catálogo (el menor entre precio_venta y precio_mayoreo, si este
                    // último está configurado), aplicando exactamente la misma regla del 50%/100%
                    // que ya existía — no se endurece nada para productos que sí tienen costo.
                    ...
                    if (!$esPrecioPromoValido) {
                        if ($referenciaFloor > 0) {
                            ...
                            // Producto sin ningún precio de referencia capturado (compra/venta/mayoreo
                            // en $0) — no hay forma de validar proporción, pero un precio de $0 o
                            // negativo nunca es válido para una venta.
                            throw new Exception("Precio inválido para el producto. Verifica el carrito.");
                        }
                    }
                }
                ...
                $stmtS->execute([$item['producto_id'], $sucursalVista]);
                ...
                $pdo->prepare("UPDATE stock_sucursal SET stock_actual = ? WHERE producto_id = ? AND sucursal_id = ?")->execute([$stockNuevo, $item['producto_id'], $sucursalVista]);
            ...
            header('Location: cajero_nuevaVenta.php?msg=exito&venta_id='.$venta_id);
            exit();
...
// endpoints AJAX de arriba (que no llegan hasta este punto, todos terminan en su propio
// exit()). Cubre la carga normal de la pagina (sin AJAX) y cualquier POST que no haya
// hecho su propio exit() todavia.
if (!$caja) {
    header('Location: cajero_abrirCaja.php?msg=sinCaja');
    exit();
}
...
<?php renderAdminSidebar('cajero_nueva_venta'); ?>

<div class="main">
    <div class="topbar">
        <div class="topbar-left">
            <button class="toggle-btn" onclick="toggleSidebar()"><?= icono('menu') ?></button>
            <h2>Nueva Venta — <?= htmlspecialchars($nombreSucursalVista) ?> — Turno #<?= $caja['numero_turno'] ?></h2>
        </div>
            ...
            <div class="modal-filtros">
                ...
                <select id="invSucursal" onchange="buscarInventario()">
                    <?php foreach ($sucursales as $s): ?>
                        <option value="<?= $s['sucursal_id'] ?>" <?= $s['sucursal_id']==$sucursalVista?'selected':'' ?>>
...
const miSucursalId    = <?= intval($sucursalVista) ?>;
...
(function() {
    ...
    Promise.all([
        fetch('cajero_nuevaVenta.php?get_productos_all=1').then(r => r.json()),
        fetch('cajero_nuevaVenta.php?get_paquetes=1').then(r => r.json()),
        fetch('cajero_nuevaVenta.php?get_clientes_all=1').then(r => r.json())
...
function procesarScan(codigo) {
    ...
    fetch(`cajero_nuevaVenta.php?scan_codigo=${encodeURIComponent(codigo)}`)
        .then(r => r.json())
        .then(prod => {
            if (!prod) {
...
document.getElementById('formVenta').addEventListener('submit', function(e) {
    ...
    fetch('cajero_nuevaVenta.php', { method: 'POST', body: fd })
        .then(r => r.json())
        .then(res => {
...
function imprimirTicket(ventaId) {
    if (!ventaId) return;
    fetch(`cajero_nuevaVenta.php?ticket_venta=${ventaId}`)
        .then(r => r.json())
...
function generarTicketHTML(venta) {
    ...
    let _pie;
    if (_metodo === 'Credito')       _pie = datosTicket.ticket_pie_credito;
...
function buscarInventario() {
    ...
    toInv = setTimeout(() => {
        ...
        fetch(`cajero_nuevaVenta.php?inventario_sucursal=${sucursal}&buscar_inv=${encodeURIComponent(buscar)}`)
