...
// redirect-based (cortar ahi es consistente), pero get_ticket_venta responde JSON y un
// redirect aqui rompia su fetch().then(r=>r.json()). El guard de "liquidar" vive junto a su
// propio uso mas abajo; el de pagina completa, al final del archivo antes del HTML.
...
if (isset($_GET['liquidar'])) {
    ...
    // a $cajaActualId) y responde por redirect, no JSON — cortar aqui es consistente.
    if (!$cajaActualId) {
        header('Location: cajero_abrirCaja.php?msg=sinCaja');
        exit();
    }
...
if (isset($_GET['cancelar'])) {
    ...
    // [FIX-STOCK-PENDIENTE-INMEDIATO] El stock SÍ se descuenta ahora al crear la pendiente
    // (envío ya comprometido), así que cancelarla debe devolverlo -- antes este comentario
    // decía lo contrario porque el stock nunca se tocaba hasta "liquidar".
    $pdo->beginTransaction();
    try {
        ...
        foreach ($stmtItemsCancel->fetchAll(PDO::FETCH_ASSOC) as $itC) {
            ...
            $stmtStC->execute([$itC['producto_id'], $sucursalVista]);
            ...
            try {
                ...
                foreach ($items as &$it) {
                    ...
                    $stmtActivoChkPend = $pdo->prepare("
                        ...
                        WHERE p.producto_id = ?
                    ");
                    $stmtActivoChkPend->execute([$sucursalVista, intval($it['producto_id'] ?? 0)]);
                    ...
                    // [FIX-PISO-PENDIENTE-50] (espejo de cajeroInventario/ventasPendientes.php)
                    // El piso seguia en 50% del precio de compra mientras que cajero_nuevaVenta.php
                    // ya lo tiene en 100% completo — se alinea al mismo piso y a la misma excepcion
                    // por promocion vigente.
                    ...
                    if (!$pqIdVal) {
                        ...
                        ");
                        $stmtPromoChkPend->execute([intval($it['producto_id'] ?? 0), $sucursalVista]);
                ...
                if ($metodo_pago === 'Credito' && $cliente_id) {
                    ...
                    // [FIX-CREDITO-DOMICILIO-CONCURRENTE] (espejo de cajeroInventario/ventasPendientes.php)
                    // Un credito de venta a domicilio solo se inserta en `creditos` AL LIQUIDAR, no
                    // al crear el pedido -- dos pedidos a credito del mismo cliente, creados antes
                    // de liquidar cualquiera de los dos, pasaban esta validacion cada uno por
                    // separado. Se suman tambien los pedidos 'Pendiente' a credito de este cliente.
                ...
                // pendiente -- solo se validaba con un cálculo de "comprometido" (stock_actual
                // menos lo reservado por otras pendientes de los últimos 3 días) que vivía
                // SOLO aquí. Ninguna otra pantalla (nuevaVenta.php, inventario_productos.php,
                // el buscador de productos, etc.) sabía de esa reserva -- todas seguían
                // mostrando/vendiendo el stock_actual crudo como si estuviera 100% disponible.
                // Confirmado con el usuario: al ser un envío a domicilio ya comprometido, el
                // stock debe descontarse de una vez (igual que una venta normal), no quedar
...
// de renderizar la pagina completa — para no romper el contrato JSON de get_ticket_venta (ni
// bloquear "cancelar", que no depende de caja). Cubre la carga normal de la pagina.
if (!$cajaActualId) {
    header('Location: cajero_abrirCaja.php?msg=sinCaja');
    exit();
}
...
let carritoP = (function() {
    // [FIX-CARRITO-CROSS-SUCURSAL 2026-09-12] (mismo hueco encontrado en cajero_nuevaVenta.php)
    // el Administrador puede cambiar de sucursal a medio armado de un apartado -- sin este
    // candado, restauraba productos con el precio/promocion/stock de la sucursal VIEJA.
    try {
...
function renderCarritoMini() {
    ...
    localStorage.setItem('carritoPendiente_sucursal_id', String(<?= intval($sucursalVista) ?>));
...
function generarTicketHTML(venta) {
    ...
    // [FIX-REFERENCIA-TICKET] (portado de cajero_nuevaVenta.php): el ticket nunca mostraba la
    // referencia bancaria de una venta por Transferencia, aunque ya se guardaba correctamente
    // en la base de datos (venta.referencia_transferencia, incluida en el SELECT de arriba).
