...
fetch('cajero_nuevaVenta.php?get_paquetes=1')
    .then(r => r.json())
    .then(data => {
...
document.getElementById('inputProducto').addEventListener('input', function() {
    ...
    toProd = setTimeout(() => {
        Promise.all([
            fetch(`cajero_nuevaVenta.php?buscar_producto=${encodeURIComponent(val)}&sucursal_id=${miSucursalId}`).then(r=>r.json()),
            fetch(`cajero_nuevaVenta.php?buscar_paquete=${encodeURIComponent(val)}`).then(r=>r.json())
...
document.getElementById('inputCliente').addEventListener('input', function() {
    ...
    toCli = setTimeout(() => {
        fetch(`cajero_nuevaVenta.php?buscar_cliente=${encodeURIComponent(val)}`)
