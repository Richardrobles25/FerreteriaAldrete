...
verificarSesion();
// [REWRITE-ABONOS] Modulo exclusivo del Administrador (a diferencia de cajero_creditos.php,
// que tambien lo puede usar un Cajero/Inventario-Cajero desde el panel admin).
...
// Caja abierta del usuario actual EN LA SUCURSAL ELEGIDA — si no hay, redirigir a abrirCaja
$stmtCajaCheck = $pdo->prepare("SELECT caja_id FROM cajas WHERE usuario_id = ? AND sucursal_id = ? AND estado = 'Abierta' LIMIT 1");
$stmtCajaCheck->execute([$_SESSION['usuario_id'], $sucursalVista]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'registrar_abono') {
    ...
    try {
        if ($sucursalVista === 0) throw new Exception('Selecciona una sucursal específica para registrar un pago.');
        $stmtCajaAb = $pdo->prepare("SELECT caja_id FROM cajas WHERE usuario_id = ? AND sucursal_id = ? AND estado = 'Abierta' LIMIT 1");
        $stmtCajaAb->execute([$_SESSION['usuario_id'], $sucursalVista]);
        ...
        if ($metodo === 'Mixto') {
            if ($monto_ef > 0) {
                $pdo->prepare("INSERT INTO movimientos_caja (caja_id, usuario_id, sucursal_id, tipo, monto, nota) VALUES (?,?,?,'Ingreso',?,?)")
                    ->execute([$cajaIdAb, $_SESSION['usuario_id'], $sucursalVista, $monto_ef, $notaBase . ' [Efectivo]']);
            }
            if ($monto_term > 0) {
                $pdo->prepare("INSERT INTO movimientos_caja (caja_id, usuario_id, sucursal_id, tipo, monto, nota) VALUES (?,?,?,'Ingreso',?,?)")
                    ->execute([$cajaIdAb, $_SESSION['usuario_id'], $sucursalVista, $monto_term, $notaBase . ' [Terminal]']);
            }
        } else {
            $montoMov  = ($metodo === 'Terminal') ? $monto + $comisionTotal : $monto;
            $pdo->prepare("INSERT INTO movimientos_caja (caja_id, usuario_id, sucursal_id, tipo, monto, nota) VALUES (?,?,?,'Ingreso',?,?)")
                ->execute([$cajaIdAb, $_SESSION['usuario_id'], $sucursalVista, $montoMov, $notaBase . ' [' . $metodo . ']']);
        }
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        error_log('[Ferreteria/abonos] Error PDO abono: ' . $e->getMessage());
...
// [REWRITE-ABONOS] AJAX: cancelar la ultima mora cobrada a un credito (exclusivo Administrador).
// Solo descuenta el monto de la mora mas reciente, nunca el historial completo, y exige un
// motivo obligatorio que queda guardado en mora_cancelaciones para auditoria. No cambia el
// estado "Vencido" del credito (solo pasa a Liquidado en el caso extremo de que la mora fuera
// justo lo unico que faltaba).
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'cancelar_mora') {
    header('Content-Type: application/json');
    requerirCSRF($_POST['_token'] ?? '', 'abonos.php');
    try {
        $cred_id_mora = intval($_POST['credito_id'] ?? 0);
        $motivoCancel = trim($_POST['motivo_cancelacion'] ?? '');
        if (!$cred_id_mora) throw new Exception('Crédito no válido.');
        if ($motivoCancel === '') throw new Exception('El motivo es obligatorio para cancelar la mora.');

        $pdo->beginTransaction();
        $stmtLockMora = $pdo->prepare("SELECT saldo_pendiente, mora_acumulada, estado FROM creditos WHERE credito_id = ? FOR UPDATE");
        $stmtLockMora->execute([$cred_id_mora]);
        $credLockMora = $stmtLockMora->fetch(PDO::FETCH_ASSOC);
        if (!$credLockMora) throw new Exception('Crédito no encontrado.');

        $moraActual = round(floatval($credLockMora['mora_acumulada']), 2);
        if ($moraActual <= 0) throw new Exception('Este crédito no tiene mora activa que cancelar.');

        $nuevoSaldoMora  = max(0, round(floatval($credLockMora['saldo_pendiente']) - $moraActual, 2));
        $nuevoEstadoMora = $nuevoSaldoMora <= 0.001 ? 'Liquidado' : $credLockMora['estado'];

        $pdo->prepare("UPDATE creditos SET saldo_pendiente = ?, mora_acumulada = 0, estado = ? WHERE credito_id = ?")
            ->execute([$nuevoSaldoMora, $nuevoEstadoMora, $cred_id_mora]);
        $pdo->prepare("INSERT INTO mora_cancelaciones (credito_id, usuario_id, monto, motivo) VALUES (?,?,?,?)")
            ->execute([$cred_id_mora, $_SESSION['usuario_id'], $moraActual, $motivoCancel]);

        $pdo->commit();
        echo json_encode(['ok' => true]);
    } catch (\PDOException $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
        error_log('[Ferreteria/abonos] Error PDO cancelar mora: ' . $e->getMessage());
        echo json_encode(['ok' => false, 'error' => 'No se pudo cancelar la mora. Intenta de nuevo.']);
    } catch (Exception $e) {
        if ($pdo->inTransaction()) $pdo->rollBack();
        echo json_encode(['ok' => false, 'error' => $e->getMessage()]);
    }
...
if (isset($_GET['get_creditos_cliente'])) {
    ...
    try {
        ...
        foreach ($creditos as &$cr) {
            ...
            // [REWRITE-ABONOS] Historial de moras canceladas por un administrador (auditoria).
            $stmtMC = $pdo->prepare("
                SELECT mc.monto, mc.motivo, mc.created_at, u.nombre_completo AS admin_nombre
                FROM mora_cancelaciones mc
                JOIN usuarios u ON mc.usuario_id = u.usuario_id
                WHERE mc.credito_id = ?
                ORDER BY mc.created_at ASC
            ");
            $stmtMC->execute([$cr['credito_id']]);
            $cr['mora_cancelada'] = $stmtMC->fetchAll(PDO::FETCH_ASSOC);
        }
        echo json_encode($creditos);
    } catch (\Throwable $e) {
        error_log('[Ferreteria/abonos] Error get_creditos: ' . $e->getMessage());
...
// [REWRITE-ABONOS] Deep-link: admin/creditos.php sigue enlazando "Ver abonos" como
// abonos.php?ver=<credito_id> — se resuelve a que cliente pertenece ese credito para abrir
// su modal automaticamente al cargar la pagina, igual que si lo hubieras buscado a mano.
$deepLinkCliente = null;
$verCreditoId = intval($_GET['ver'] ?? 0);
if ($verCreditoId) {
    $stmtDl = $pdo->prepare("SELECT cr.cliente_id, c.nombre_completo FROM creditos cr JOIN clientes c ON cr.cliente_id = c.cliente_id WHERE cr.credito_id = ?");
    $stmtDl->execute([$verCreditoId]);
    $deepLinkCliente = $stmtDl->fetch(PDO::FETCH_ASSOC);
}
...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .credito-det-footer { padding: 8px 14px; border-top: 0.5px solid #eee; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 6px 14px; }
    ...
    .btn-cancelar-mora { background: none; color: #e67e22; border: 1px solid #f5c99a; padding: 3px 10px; border-radius: 5px; font-size: 11px; font-weight: 600; cursor: pointer; }
    .btn-cancelar-mora:hover { background: #fff3e0; }
        ...
        <div class="modal-body">
            ...
            <form id="formAbono" onsubmit="submitAbono(event)">
                ...
                <div id="abCajaCerrada" style="<?= $cajaAbiertaId ? 'display:none;' : '' ?>background:#fff3e0;border:1px solid #ffcc80;border-radius:6px;padding:10px 14px;margin-bottom:12px;font-size:13px;color:#e65100;">
                    Sin caja abierta. <a href="cajero_abrirCaja.php" style="color:#e65100;font-weight:700;">Abrir caja →</a>
                </div>
...
function abrirDetalles(clienteId, nombre) {
    ...
    fetch('abonos.php?get_creditos_cliente=' + clienteId)
        .then(r => r.json())
        .then(creditos => {
            ...
            creditos.forEach(cr => {
                ...
                // [REWRITE-ABONOS] Historial de moras canceladas por un administrador.
                let moraCancHtml = '';
                if (cr.mora_cancelada && cr.mora_cancelada.length > 0) {
                    moraCancHtml = `<div style="padding:8px 14px;background:#f0f9e8;border-top:0.5px solid #cfe8c0;">
                        <div style="font-size:11px;font-weight:700;color:#2e7d32;margin-bottom:5px;">Moras canceladas</div>
                        <table style="width:100%;border-collapse:collapse;font-size:11px;color:#666;">
                            <tr><th style="text-align:left;padding:2px 6px 2px 0;font-weight:600;">Fecha</th><th style="text-align:left;padding:2px 6px;font-weight:600;">Motivo</th><th style="text-align:left;padding:2px 6px;font-weight:600;">Admin</th><th style="text-align:right;padding:2px 0;font-weight:600;color:#2e7d32;">Monto</th></tr>
                            ${cr.mora_cancelada.map(m => `<tr>
                                <td style="padding:2px 6px 2px 0;">${esc(m.created_at ? m.created_at.substring(0,10) : '')}</td>
                                <td style="padding:2px 6px;">${esc(m.motivo)}</td>
                                <td style="padding:2px 6px;">${esc(m.admin_nombre)}</td>
                                <td style="text-align:right;color:#2e7d32;font-weight:600;">-$${parseFloat(m.monto).toFixed(2)}</td>
                            </tr>`).join('')}
                        </table>
                    </div>`;
                }
...
// [REWRITE-ABONOS] Cancelar la última mora cobrada a un crédito — exige motivo obligatorio y
// refresca el modal con los datos actualizados sin recargar toda la página.
function cancelarMora(creditoId, montoMora) {
    const motivo = prompt('Motivo para cancelar la mora de +$' + montoMora.toFixed(2) + ' (obligatorio):');
    if (motivo === null) return;
    if (motivo.trim() === '') { alert('El motivo es obligatorio para cancelar la mora.'); return; }

    const tok = document.querySelector('#formAbono input[name="_token"]').value;
    const body = new URLSearchParams();
    body.set('_token', tok);
    body.set('action', 'cancelar_mora');
    body.set('credito_id', creditoId);
    body.set('motivo_cancelacion', motivo.trim());

    fetch('abonos.php', { method: 'POST', body })
        .then(r => r.json())
        .then(res => {
            if (res.ok) {
                abrirDetalles(_clienteIdActual, document.getElementById('modalTitulo').textContent);
            } else {
                alert(res.error || 'Error al cancelar la mora.');
            }
        })
        .catch(() => alert('Error de conexión al cancelar la mora.'));
}
...
function cargarHistorialPagos(clienteId) {
    ...
    fetch('abonos.php?get_abonos_cliente=' + clienteId)
        .then(r => r.json())
        .then(abonos => {
            if (!abonos.length) {
...
function submitAbono(e) {
    ...
    fetch('abonos.php', { method: 'POST', body: data })
        .then(r => r.json())
        .then(res => {
            if (res.ok) {
...
<?php if ($deepLinkCliente): ?>
// [REWRITE-ABONOS] Entrada directa desde admin/creditos.php ("Ver abonos" -> ?ver=<credito_id>)
