...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errores)) {
        $stmtP = $pdo->prepare("SELECT ss.stock_actual, p.nombre_producto FROM productos p INNER JOIN stock_sucursal ss ON ss.producto_id = p.producto_id AND ss.sucursal_id = ? WHERE p.producto_id = ? AND p.activo = 1");
...
$stmt = $pdo->prepare("SELECT p.producto_id, p.codigo, p.nombre_producto, ss.stock_actual FROM productos p INNER JOIN stock_sucursal ss ON ss.producto_id = p.producto_id AND ss.sucursal_id = ? WHERE p.activo = 1 AND ss.activo = 1 AND ss.stock_actual > 0 ORDER BY p.nombre_producto ASC");
$stmt->execute([$_SESSION['sucursal_id']]);
