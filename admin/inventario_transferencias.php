...
if (isset($_accionData['accion']) && (isset($_accionData['id']) || isset($_GET['id']))) {
    // Verificar CSRF una sola vez: todas las acciones de este bloque
    ...
    if ($miSucursal === 0) {
        header('Location: inventario_transferencias.php?msg=error_sucursal'); exit();
    }
    ...
    if ($accion === 'editar_cantidad') {
        ...
        if ($nuevaCantidad > 0) {
            ...
            if ($nuevaCantidad <= 0) { header('Location: inventario_transferencias.php?msg=cantidad_editada'); exit(); }
            if ($tvRow && $tvRow['stock_origen'] !== null && $nuevaCantidad > floatval($tvRow['stock_origen'])) {
                header('Location: inventario_transferencias.php?msg=error_stock_editar'); exit();
            }
        ...
        if ($transf) {
            $pdo->beginTransaction();
            try {
                // Volver a leer y bloquear la transferencia dentro de la transaccion (mismo
                // patron que en 'recibir'): un doble clic/doble envio no debe descontar dos veces.
                ...
                if ($stockOr === false || floatval($stockOr) < floatval($transf['cantidad'])) {
                    $pdo->rollBack();
                    header('Location: inventario_transferencias.php?msg=error_stock_envio'); exit();
                }
        ...
        if ($transf && $transf['estado'] === 'En tránsito' && $transf['sucursal_destino_id'] == $miSucursal) {
            $pdo->beginTransaction();
            try {
                // Volver a leer y bloquear la transferencia dentro de la transaccion: si dos
                // peticiones (doble clic) llegan casi juntas, la segunda encuentra la
                // transferencia ya "Entregada" y no hace nada.
                ...
                // las que quedaron "En tránsito" antes de este cambio no, y hay que descontarlo aqui)
                ...
                if (!$origenYaDescontado) {
                    ...
                    if ($stockOrLegacy === false || floatval($stockOrLegacy) < floatval($transf['cantidad'])) {
                        $pdo->rollBack();
                        header('Location: inventario_transferencias.php?msg=error_stock_recibir'); exit();
                    }
...
// Transferencias relevantes: si hay una sucursal elegida, como origen o destino de esa
// sucursal; si es "todas las sucursales" (0), se muestran todas.
$whereSuc = '';
$paramsSuc = [];
if ($sucursalVista !== 0) {
    $whereSuc = ' AND (t.sucursal_origen_id = ? OR t.sucursal_destino_id = ?)';
    $paramsSuc = [$sucursalVista, $sucursalVista];
}
...
$stmt = $pdo->prepare("
    ...
    LIMIT 100
");
$stmt->execute(array_merge($paramsSuc, $paramsExtra));
$transferencias = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Sucursales origen disponibles (todas menos la elegida)
...
if (isset($_GET['exportar']) && in_array($_GET['exportar'], ['pdf','excel'])) {
    ...
    // [FIX] Respetar el filtro de sucursal (antes exportaba TODAS las sucursales sin importar
    ...
    $stmtExp = $pdo->prepare("
        ...
        ORDER BY t.created_at DESC
    ");
    $stmtExp->execute(array_merge($paramsSuc, $paramsExtra));
    ...
    <div class="content">
                ...
                <table>
                    ...
                    <tbody>
                        <?php foreach ($transferencias as $t):
                            $esMiOrigen = $t['sucursal_origen_id'] == $sucursalVista;
                            $badgeMap = [
        ...
        <div>
            <div class="card">
                ...
                <p style="font-size:12px;color:#aaa;margin:-8px 0 14px;">Pide productos de otra sucursal a la que elegiste arriba.</p>

                <?php if ($sucursalVista === 0): ?>
                    <div class="msg" style="background:#fff8e1;color:#e65100;">Selecciona una sucursal específica arriba para poder solicitar transferencias.</div>
                <?php else: ?>
...
const _busquedaProdEl = document.getElementById('busquedaProd');
if (_busquedaProdEl) {
    _busquedaProdEl.addEventListener('blur', function() {
...
// restaurarEstadoVenta() en cajero_nuevaVenta.php. Se lee "extra" ANTES de renderItems() por
...
<form id="formEditarCantidadTransf" method="POST" action="inventario_transferencias.php" style="display:none;">
    <input type="hidden" name="_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
